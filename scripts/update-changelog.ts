...
import { generateMarkDown, getCurrentGitBranch, loadChangelogConfig, determineSemverChange, getGitDiff, parseCommits } from 'changelogen'

const repo = `nuxt-contrib/vue-bundle-renderer`
const pkgName = `vue-bundle-renderer`
...
async function main() {
  ...
  const newVersion = inc(workspace.find(pkgName).data.version, bumpType)
...
export async function loadPackage(dir: string) {
  ...
  function updateDeps(reviver: (dep: Dep) => Dep | void) {
...
export async function getContributors() {
  ...
  for (const commit of rawCommits) {
    ...
    const { author } = await fetch(`https://api.github.com/repos/${repo}/commits/${commit.shortHash}`, {
      headers: {
        'User-Agent': `${repo} GitHub Actions release script`,
